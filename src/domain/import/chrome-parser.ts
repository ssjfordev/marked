...
// Skip these special Chrome folders (they're virtual)
const SKIP_FOLDERS = new Set(['Bookmarks', 'Other Bookmarks', 'Mobile Bookmarks']);
  ...
  for (const line of lines) {
    ...
    if (folderMatch) {
      ...
      if (!SKIP_FOLDERS.has(folderName)) {
        ...
        // Still need to track depth for proper closing
        folderStack.push('__SKIP__');
        folderStackNodes.push([]);
      }
      continue;
    }
