...
function FavoriteLinksContent({ links: initialLinks }: FavoriteLinksProps) {
  ...
  const handleToggleFavorite = (linkId: string) => {
    // When unfavoriting, remove from list immediately
    setLinks((currentLinks) => currentLinks.filter((l) => l.id !== linkId));
  ...
  const handleBulkFavorite = useCallback(
    async (ids: string[], favorite: boolean) => {
      if (!favorite) {
        // Unfavoriting: remove from favorites list
        setLinks((currentLinks) => currentLinks.filter((l) => !ids.includes(l.id)));
      } else {
        setLinks((currentLinks) =>
          currentLinks.map((link) =>
            ids.includes(link.id) ? { ...link, is_favorite: true } : link
          )
        );
      }
